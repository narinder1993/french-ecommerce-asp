         ...
         protected void Page_Load(object sender, EventArgs e)
         {
             try
             {
                 ...
                 try
                 {
                     ...
                     cat = arrQueryStrings[0];
                     page = arrQueryStrings[1];

                     var catStrings = cat.Split('=');
                     var pageQueryStrings = page.Split('=');
                     cat = catStrings[1];
                     int page1 = Convert.ToInt32(pageQueryStrings[1]);
                     ...
                     System.Data.SqlClient.SqlCommand cmd8 = new System.Data.SqlClient.SqlCommand(";with A aS(SELECT  id, nameEng, ctgEng, descEng, price, image1 isAvlbl, sizes,ROW_NUMBER() OVER (ORDER BY Id) AS 'RN' from products where ctgEng like '" + cat + "' )	select * from A where RN between " + ((page1 * 12) - 11) + " and " + page1 * 12 + " ", con);
                     ...
                     foreach (System.Data.DataRow dr in ds.Tables[0].Rows)
                     {
                         ...
                         cstext2.Append("<span class='buy-in'  href='single.aspx?id=" + dr[0].ToString() + "'>Buy Now</span>");
                         cstext2.Append("</div>");
                         cstext2.Append("</a>	");
                         cstext2.Append("</div>");
                     ...
                     System.Data.SqlClient.SqlCommand cmd9 = new System.Data.SqlClient.SqlCommand("select top 2 id, nameEng, price, image1 from products order by newid() ", con);
